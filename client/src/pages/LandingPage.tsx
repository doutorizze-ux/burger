...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 selection:bg-slate-1000 selection:text-white">
      {/* Header */}
      <header className="fixed w-full top-0 bg-white/80 backdrop-blur-md z-50 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-red-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-black/20">🍔</div>
            <h1 className="font-extrabold text-2xl tracking-tight">PitDog<span className="text-black">.ai</span></h1>
          </div>
          <div className="flex gap-4">
            <a href="/login" className="font-bold text-slate-600 hover:text-slate-900 px-4 py-2">Fazer Login</a>
            <a href="/login?register=true" className="bg-black text-white font-bold px-6 py-2.5 rounded-full hover:bg-slate-800 transition-all shadow-md">Criar minha Conta</a>
          </div>
        </div>
      </header>
      ...
      <section className="pt-40 pb-20 px-6 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
        <div className="lg:w-1/2 space-y-8">
          <div className="inline-flex items-center gap-2 bg-orange-100 text-orange-700 font-bold px-4 py-2 rounded-full text-sm">
            <span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary border-black border opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-slate-1000"></span></span>
          ...
          <div className="flex gap-4 pt-4">
            <a href="/login?register=true" className="bg-primary text-black font-black text-lg px-8 py-4 rounded-full hover:bg-primary border-black border transition-all shadow-xl shadow-black/20 flex items-center gap-2">Testar 7 dias Grátis <ChevronRight/></a>
          </div>
      ...
      <section className="py-24 bg-white border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-12">
          <div className="space-y-4">
            <div className="w-16 h-16 bg-blue-100 text-black rounded-2xl flex items-center justify-center"><Bot size={32}/></div>
          ...
          <div className="space-y-4">
            <div className="w-16 h-16 bg-orange-100 text-black rounded-2xl flex items-center justify-center"><Zap size={32}/></div>
      ...
      <section className="py-24 bg-slate-900 text-center px-6">
        ...
        <a href="/login?register=true" className="inline-block bg-primary text-black font-black text-xl px-10 py-5 rounded-full hover:bg-primary border-black border transition-all shadow-[0_0_40px_rgba(249,115,22,0.4)]">Quero Criar Minha Loja!</a>
      </section>
    </div>
  );
}
